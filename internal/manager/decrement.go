...
func (m *Manager) handleDecrement(ctx context.Context, command serializer.Array) serializer.RESPType {
	...
	if err != nil {
		m.logger.With(logging.FieldsFromContext(ctx)...).Warn("failed to decrement value", "key", key, "error", err)
